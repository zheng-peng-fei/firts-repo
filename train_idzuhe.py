...
# wandb.config = state
wandb.init(project="OOD", entity="2465607784", config=state)
...
def train(epoch):
    ...
    for in_set, aux_in_set, aux_out_set in loaders:
        ...
        tensor0=tensordata[0:64]
        tensor0=tensor0.reshape(196608)
        tensor1=tensordata[64:128]
        tensor1=tensor1.reshape(196608)

        mask = rng.choice(a=[False, True], size=(196608), p=[0.5, 0.5])
        tensor0_subsampled0 = tensor0[mask]
        tensor0_subsampled1 = tensor0[np.invert(mask)]
        tensor1_subsampled0 = tensor1[np.invert(mask)]
        tensor1_subsampled1 = tensor1[mask]

        tensor_0 = torch.cat((tensor0_subsampled0, tensor1_subsampled0), 0)
        tensor_1 = torch.cat((tensor0_subsampled1, tensor1_subsampled1), 0)
        tensor=torch.cat((tensor_0,tensor_1),0)
        tensor=tensor.reshape(128,3,32,32)

        data = torch.cat((in_set[0], tensor), 0)
        target = in_set[1]
        ...
        if args.score == 'woods_nn':
            ...
            loss = loss_ce + args.out_constraint_weight*loss_energy_out + in_loss #in_constraint_weight和ce_constraint_weight哪个大于1哪个增大对应的
